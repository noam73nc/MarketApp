...
with st.expander("📖 מדריך קריאה למשתמש (Cheat Sheet) - לחץ לפתיחה", expanded=False):
    st.markdown("""
    ### ברוכים הבאים ל-Strike Zone Command Center! 🎯
    מערכת זו סורקת אוטומטית אלפי מניות בבורסה האמריקאית ומזהה תבניות מחיר מוסדיות בזמן אמת.
    
    **מילון תבניות מחיר (Pattern Badges):**
    * **👑 True VCP:** הגביע הקדוש! מניה שעברה אימות מתמטי קפדני לפי חוקי מארק מינרוויני (התכווצות תנודתיות + התייבשות מחזורים).
    * **EP 🚀:** Episodic Pivot. המניה פתחה היום ב"גאפ" (פער) של למעלה מ-10% בעקבות אירוע/דוחות.
    * **Gap 📈:** קפיצת מחיר סטנדרטית של 4% ומעלה בפתיחה.
    * **Tight 🤏:** יום מסחר שקט במיוחד - טווח המסחר התכווץ ב-40% מתחת לממוצע עם מחזור נמוך. הכנה לפריצה.
    * **2 ADR 🔥:** תנודתיות מטורפת. המניה זזה היום פי 2 מהתנועה הממוצעת הרגילה שלה!
    * **SQUAT 🏋️:** תמרור אזהרה. יום עם מחזור גבוה אבל המניה נסגרה בנמוך היומי שלה (פריצת שווא / מוכרים כבדים).
    
    **איך משתמשים בלוח?**
    1. השתמשו ב**תפריט הצד (הסיידבר)** כדי לסנן מניות לפי שווי שוק, כמות מסחר (RVOL), ותבניות ספציפיות.
    2. עמודת ה-**SCORE 🎯** מתעדפת את המניות עם הסטאפים האיכותיים ביותר.
    3. לחיצה על הקישור בעמודת **SYM 🔗** תפתח לכם את הגרף המלא ב-TradingView.
    4. ניתן לייצא את הלוח הנוכחי ל-**Excel** בכל רגע דרך הכפתור בסיידבר!
    """)
