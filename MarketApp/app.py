...
with st.expander("📖 מדריך קריאה למשתמש (Cheat Sheet) - לחץ לפתיחה", expanded=False):
    ...
    <p>מערכת זו סורקת אוטומטית אלפי מניות בבורסה האמריקאית ומזהה תבניות מחיר מוסדיות בזמן אמת.</p>
    
    <p><b>מילון תבניות מחיר (Pattern Badges):</b></p>
    <ul>
        <li><b>👑 True VCP:</b> הגביע הקדוש! מניה שעברה אימות מתמטי קפדני לפי חוקי מארק מינרוויני (התכווצות תנודתיות + התייבשות מחזורים).</li>
        <li><b>EP 🚀:</b> Episodic Pivot. המניה פתחה היום ב"גאפ" (פער) של למעלה מ-10% בעקבות אירוע/דוחות.</li>
        ...
        <li><b>U&R 🛡️:</b> Under & Reversal (ניעור). המניה ירדה מתחת לממוצע נע קריטי (כמו 21 או 50), "ניערה" ידיים חלשות, אבל נדחפה חזרה למעלה לסגירה מעל הממוצע.</li>
        <li><b>Tight 🤏:</b> יום מסחר שקט במיוחד - טווח המסחר התכווץ משמעותית מתחת לממוצע עם מחזור נמוך. הכנה לפריצה.</li>
        <li><b>VDU 🏜️:</b> Volume Dry-Up (התייבשות מחזורים). ירידה חדה במחזור המסחר המעידה על כך שאין יותר מוכרים בשוק (ההיצע התייבש).</li>
        ...
        <li><b>SQUAT 🏋️:</b> תמרור אזהרה. יום עם מחזור גבוה אבל המניה נסגרה בנמוך היומי שלה (פריצת שווא / מוכרים כבדים שמדכאים את המחיר).</li>
    </ul>
    
    <p><b>איך משתמשים בלוח?</b></p>
    <ol>
        <li>השתמשו ב<b>תפריט הצד (הסיידבר)</b> כדי לסנן מניות לפי שווי שוק, כמות מסחר (RVOL), ותבניות ספציפיות.</li>
